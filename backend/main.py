...
def handle_browser(message: str):
    ...
    for site, url in sites.items():
        if f"open {site}" in msg:
            return {"message": f"Opening {site}", "action": "open_url", "url": url}
...
async def chat(req: ChatRequest):
    ...
    if client is None:
        return {"message": "AI chat is currently unavailable. Set the OPENROUTER_API_KEY environment variable to enable conversational responses. In the meantime, try asking for the time, date, a joke, or to open YouTube/Google/GitHub."}
...
async def upload(
    ...
    if client is None:
        ...
        return {
            ...
            "filename": safe_name, "size": size, "mime": mime,
        }
    ...
    try:
        ...
        if is_text:
            ...
            prompt_text = user_note or "Summarize this file and point out anything important."
        ...
        return {
            ...
            "filename": safe_name, "size": size, "mime": mime, "error": str(e),
        }
